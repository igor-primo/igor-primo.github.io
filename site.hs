...
main = hakyllWith config $ do
  ...
  match ("apresentacao.txt" .||. "training.html" .||. "training-goal.html" .||. "qualidade-dados.hmm.html") $ do
    route idRoute
    compile copyFileCompiler
  ...
  create ["en/archive.html"] $ do
    ...
    compile $ do
      ...
      let archiveCtx =
            listField "posts" postCtxEn (return posts)
  ...
  create ["pt/archive.html"] $ do
    ...
    compile $ do
      ...
      let archiveCtx =
            listField "posts" postCtxPt (return posts)
              `mappend` constField "title" "Arquivos"
              `mappend` defaultContext
  ...
  match "en/index.html" $ do
    ...
    compile $ do
      ...
      let indexCtx =
            listField "posts" postCtxEn (return posts)
              `mappend` constField "title" "My Minimal Presence"
              `mappend` defaultContext
  ...
  match "pt/index.html" $ do
    ...
    compile $ do
      ...
      let indexCtx =
            listField "posts" postCtxPt (return posts)
              `mappend` constField "title" "Minha presença mínima"
              `mappend` defaultContext
...
postCtxEn =
  dateField "date" "%m/%d/%Y"
    `mappend` field "enUrl" insertEnUrl
    `mappend` field "ptUrl" insertPtUrl
    `mappend` defaultContext
